...
- **FTS5 search:** Unified into `GET /api/articles?q=...`. When `q` is present, uses `INNER JOIN articles_fts` with `ORDER BY articles_fts.rank` (unless `sort` is explicit). All filters (`tag`, `reading_status`, etc.) compose with search. No separate search endpoint.
